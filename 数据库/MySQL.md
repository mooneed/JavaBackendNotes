...
>Tips：否则很多场景下都会埋坑，如：频繁触发死锁、高主从延迟、大量慢查询等问题。

**MySQL网课**：MySQL45讲-丁奇（**`MySQL核心参考资料`**）（学习曲线较平缓，有深度，不过知识点间的分离度较低，需要较多时间学习）  
**MySQL博客**：
...
### 写入机制
binlog的写入性能并不高（从默认关闭binlog的设置可以略窥一斑）。可以从MySQL如何提高binlog性能的角度，来学习其写入机制。  
>Tips：binlog使用了缓存技术，并通过sync_binlog参数控制缓存使用策略，平衡性能&丢失数据风险的容忍程度。  
...
##MySQL性能问题的原因
（1）**硬件问题**：CPU内存磁盘（加钱），网络延迟（加钱）。 
解决方案：加钱。   
（2）**慢查询问题**：索引问题（设计阶段，执行阶段），缓存命中率问题，读写锁冲突问题，。  
解决方案：。  
>Tips：在MySQL上不能较好解决查询性能问题时，可以引入查询中间件ES、引入本地缓存/远程缓存等外部技术提高数据系统性能。  

（3）**并发问题**：并发阻塞问题，死锁问题。  
解决方案：短事务，，死锁检测。  
（4）**连接问题**：
解决方案：避免短连接，。  
（5）**负载问题**：千万级的单表会开始出现性能问题。  
解决方案：分库分表（包括冷热分离等策略）。  
>Tips：MySQL性能问题的原因&解决方案并不是孤立的，如冷热分离的分库分表策略也可以有效改善慢查询问题，慢查询问题也存在并发问题的原因。等等。  
## 慢查询优化

>Tips：索引失效时，强制使用索引force index()会失效。  
### 执行计划查询工具explain
![explain](https://github.com/mooneed/JavaBackendNotes/assets/37293242/1c32158f-1fc9-4120-83da-ef48881707be)
**字段含义**：
（1）id：sql查询语句的序列号，序列号越大，执行优先级越高；  
（2）select_type：查询复杂度，SIMPLE简单查询；  
（3）table：访问的表名；  
（4）partitons：数据分区信息；  
（5）**type**：查询类别，性能由好到差：system>const>eq_ref>ref>range>index>all；  
>type类别含义：system/const为主键索引/唯一索引查询，只匹配一条数据时可以优化成常量，eq_ref为使用主键索引/唯一索引连接查询只返回一条数据，ref使用索引或前缀（似乎只对字符串生效），range为范围查询（应该是针对主键），index为索引查询，all为全表扫描（实际为扫描主键索引树的叶子结点）。此外type为null则为当前查询语句不需要访问表。  

>Tips：最好优化到ref或range。选择的字段全部被索引覆盖则为index及更好的类别（MySQL对select * 的优化是？）  

（6）possible_keys：sql执行时可能使用到的索引（最终出于sql策略并没有使用）；  
（7）**key**：sql执行时真正用到的索引；  
（8）key_len：索引长度（最大允许长度768字节）；  
（9）ref：表示列/常来那个被用于查找索引列上的值；  
（10）**rows**：扫描行数；  
（11）filtered：表示存储引擎返回的数据在server层过滤后，满足查询的记录数量的百分比（实际参考意义不大）；  
（12）extra：额外信息。  
### sql语句耗时分析
```SQL
// 开启耗时分析
...
show profile;
```
![show profile](https://github.com/mooneed/JavaBackendNotes/assets/37293242/0d78bc2a-49b9-42ff-9613-928391bae5cc)
>Tips：由图中可见，范围查询最耗时的操作是IO（图中示例为898满足条件数据行/3004总数据行比例的开发环境自测）。  
## 并发锁问题
锁导致的各种性能问题。
### 加锁查询工具
### 安全更新
